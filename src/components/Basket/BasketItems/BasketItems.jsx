...
const BasketItems = ({item}) => {
    ...
    return (
        <NavLink className='productItem-productLink' to={'/products/' + item.id}>
            <div className='productItem'>

                <svg className='productItem-favorites' width="21" height="19" viewBox="0 0 21 19" fill="none"
                ...
                <img className='productItem-productPhoto' src={item.img} alt="Фото"/>
                <div className='productItem-block'>
                    <p className='productItem-block__title'>{item.name}</p>
                    <p className='productItem-block__price'><span>Цена: </span>{item.price} ₽</p>
                    <p className={(item.sizes.length <= 1) ? 'productItem-block__sizes-last' : 'productItem-block__sizes'}>
                        <span>Размер: </span>{item.sizes}</p>
                    <button className='productItem-block__del default-button' onClick={deleteItemsFromBasket}>
                        <img className='productItem-block__del-img' src={del} alt="Удалить товар"/>
                    </button>
                </div>
            </div>
        </NavLink>
