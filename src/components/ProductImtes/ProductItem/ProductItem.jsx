...
const ProductItem = ({product}) => {
    ...
    return (
        <NavLink className='productItem-productLink' to={'/products/' + product.id}>
            <div className='productItem'>
                <svg onClick={favoriteAddFunc} className='productItem-favorites' width="21" height="19"
                    ...
                    <path
                        ...
                        fill={favorites ? '#FF8585' : "#9B9B9B"}/>
                </svg>

                <img className='productItem-productPhoto' src={product.img} alt="Фото"/>
                <div className='productItem-block'>
                    <p className='productItem-block__title'>{product.name}</p>
                    <p className='productItem-block__price'><span>Цена: </span>{product.price} ₽</p>
                    <p className={(product.sizes.length <= 1) ? 'productItem-block__sizes-last' : 'productItem-block__sizes'}>
                        <span>Размеры: </span>{product.sizes.join(', ')}</p>
                </div>
            </div>
        </NavLink>
    )
